...
/**
 * Language selector using the Bootstrap 2.2.2 markup
 * Written by MT following the patterns of renderChildrenOf function.
 *
 * @param $langs languages array
 * @param string $output
 * @return string
 */
function renderLanguageSelector($langs, $output = '') {
  $output = '';

  foreach ($langs as $language) {
    if (!wire('page')->viewable($language)) continue; // is page viewable in this language?
    $class = ' class="dropdown-item';
    $atoggle = " class='nav-link'";
    ...
    $hreflang = wire('pages')->get('/')->getLanguageValue($language, 'name');
    $output .= "<li$class><a hreflang='$hreflang' href='$url'$atoggle>$language->title</a></li>";
  }
 ...
 * @return string
 */
function renderMobileNavbar($pa, $languages) {
    $isSearchField = false;
    $out = '';
    foreach ($pa as $child) {
        $out .= "<li><a href='$child->url'>$child->title</a>";
...
// language selector
    $out .= '<li><a href="#" data-toggle="dropdown" role="button" aria-expanded="false">'.
       _('Language').'</a>
            <ul>';
    foreach ($languages as $language) {
      $out .= '<li><a hreflang="'.wire('pages')->get('/')->getLanguageValue($language, 'name').'" href="'.wire('page')->localUrl($language).'">'.$language->title.'</a></li>';
    }
    $out .= '</ul>
          </li>';
...
function bsRenderCarousel($images) {
    ...
    foreach($images as $image) {
        ...
        $out .= '<div class="carousel-caption" style="position: relative; left: auto; right: auto; background: rgba(0,0,0,0.5);"><p>'.$image->description.'</p></div>';
        $out .= '</div>';
        $i++;
    }
    $out .= "</div>";
...
function bsRenderCards($cards) {
  ...
  foreach ($cards as $card) {
    $out .= "    <div class='card'>
                   <div class='card-block'>{$card->card_text}
                     ";
    switch ($card->type) {
    case 'button':
      $out .= "        <p class='card-text'><a class='btn btn-lg {$card->card_type}' href='{$card->button_link}' role='button' data-toggle='tooltip' data-placement='right' title='{$card->button_tooltip}'><span class='fa {$card->button_icon}'></span> {$card->button_title}</a></p>";
      break;
    default:
    }
    $out .= "        </div>
